...
then
    ...
    echo" python "$REMOTEWORKERBRIDGE" -u "$hpcUserName" -m "$hpcMachineName" -i "$inpFilesToDFT $jobDetailsFile $gaussianJobDetailsFile" -p "$projDirOnHPC" -d 5 -t s -x 17280 -k "$keyToHPC" -K md"
    ...
    echo "python "$REMOTEWORKERBRIDGE" -u "$hpcUserName" -m "$hpcMachineName" -i "$inpFilesToDFT $jobDetailsFile $gaussianJobDetailsFile" -p "$projDirOnHPC" -d "$stepWaitForHPC" -t "$unitWaitForHPC" -x "$maxWaitForHPC" -k "$keyToHPC" -K md"
