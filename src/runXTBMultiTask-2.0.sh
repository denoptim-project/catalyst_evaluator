...
# Check for Cl X-ligands. If present, we save cpu time by skipping optimization of "E" structures (Because they are equal to "A"). 
...
then
    ...
    stateTypes=("${stateTypes[@]:0:labelsIndexE}" "${stateTypes[@]:labelsIndexE + 1}")
    charge=("${charge[@]:0:labelsIndexE}" "${charge[@]:labelsIndexE + 1}")
    spinmult=("${spinmult[@]:0:labelsIndexE}" "${spinmult[@]:labelsIndexE + 1}")
    jobDetailsFile="$WORKDIR/P4_nativeXTB_skip_E.jd"
...
if [ 0 -eq "$RUNXTBLOCALLY" ] && [ "$idleFraction" -gt 29 ]; then
    echo "Local CPU is $idleFraction% idle: running xTB locally."
    runLocally=0
fi
...
then
    ...
    cd "$currentDir"
    cp $wrkDir/xtb_$molNum/*XTB_opt/*xtbtrj.xyz $wrkDir/xtb_$molNum/*XTB_opt/*XTB.out $wrkDir/xtb_$molNum/*XTB.log $wrkDir/xtb_$molNum/*_last-xTB.sdf $wrkDir
    if [ "$cleanup" == 0 ]
    then 
...
do
    ...
    lineInitLog=$(grep -n "Log file '${molNum}_$label" "$outputXTB" | head -n 1 | awk -F":" '{print $1}')
    ...
    if [[ "$label" == "A" ]] || [[ "$label" == "C" ]] || [[ "$label" == "E" ]] || [[ "$label" == "F" ]] || [[ "$label" == "L" ]] ; then
        ...
        if ! tail -n +"$lineInitLogHess" "$outputXTB" | head -n "$linesToKeepHess" | grep -q "Step 0:" ; then
            errMsg="#EvaluationXTBOutput: Results of Step 0 (Numerical Hessian) not found."
            abandon "$sdfToXTB" "$E_OPTERROR"
        fi
        cpType=$(tail -n +"$lineInitLogHess" "$outputXTB" | head -n "$linesToKeepHess" | grep -A 4 "Step 0:" | grep '\-> Critial Point:' | sed 's/-> Critial Point: //')
